...
impl MLFQ {
    ...
    pub fn add_process(&mut self, process: Process) {
        ...
        let mut priority;
        if process.priority >= self.queues.len() {
            priority = self.queues.len() - 1;
        }
        else {
    ...
    pub fn execute_process(&mut self, queue_index: usize) {
        ...
        if !self.queues[queue_index].is_empty() {
            // remove the process at idx 0, the front, from the queue
            let mut process = self.queues[queue_index].remove(0);
            
            let tq = self.time_quanta[queue_index]; // current time quant from the queue
            
            // Update remaining_time, total_executed_time, and current_time
            if process.remaining_time <= tq {
                ...
                // use the rest of the time quantam
                self.current_time += tq;
                process.remaining_time -= tq;
                process.total_executed_time += tq;

                // Move the process to a lower priority queue if it doesn't complete
                if queue_index < self.queues.len() - 1 {
                    self.queues[queue_index + 1].push(process);
                }
                else {
                    // if it is in the last queue keep it there
    ...
    pub fn priority_boost(&mut self) {
        // TODO: Implement this function
        let mut temp = Vec::new(); // temporarily holds all the process
        // go through each queue and append processes to temp
        for queue_index in 0..self.queues.len() {
            for i in 0..self.queues[queue_index].len() {
                let process = self.queues[queue_index][i].clone();
                temp.push(process); // push process to temp
            }
            self.queues[queue_index].clear(); // clears queue
        }
